...
public class PublicSignServiceImpl implements IPublicSignService {
    ...
    public ReturnData publicSign(HttpServletRequest request, HttpServletResponse resp, OrderData orderData) throws Exception {
        ...
        if("".equals(brand)|| brand.isEmpty()){
            brand = "moco";
        }
        // 账号信息
        // appid
        String appid = PropertiesUtil.getHpayValue(brand+".wechat.appId");
        // 商业号
        String mch_id = PropertiesUtil.getHpayValue(brand+".wechat.mchId");
        String key = PropertiesUtil.getHpayValue(brand+".wechat.key");
    ...
    /**
     * 根据zmall传送过来的Code和品牌,调用微信接口获取access_token、openId等信息,并将openId等信息传给Zmall
     *
     * @param code
     */
    @Override
    public ReturnData getOpenIdByCode(String code,String brand) {
        if("".equals(brand)|| brand.isEmpty()){
            brand = "moco";
        }
        //返回前台信息
        ...
        String requestUrl = String.format(
                ...
                PropertiesUtil.getHpayValue(brand+".wechat.appId"),
                PropertiesUtil.getHpayValue(brand+".wechat.secret").trim(),
                code,
